...
============================
MongoMotor allows you to define schemata for documents as this helps to reduce
...
:class:`~mongomotor.Document`. Fields are specified by adding **field
...
.. code-block:: python
    ...
    import asyncio
    from mongomotor import DynamicDocument
    ...
    ...     await page.save()
    ...     print((await Page.objects(tags='mongoengine).count()))
    ...
...
scenes, and dereferenced when the :class:`Page` object is retrieved. When
getting a reference field you need to use ``await`` in it as the dereferencing
is done asynchronouslly.
...
.. code-block:: python
    ...
    await bob = User(name="Bob Jones").save()
    await john = User(name="John Smith").save()
    ...
    # To get the references in a list you must to consume the future.
    page = await Page.objects.get(title="Test Page")
    autors = await page.autors
...
method::
    ...
    await recipient.save()               # will raise a ValidationError while
    await recipient.save(validate=False) # won't

Document collections
====================
Document classes that inherit **directly** from :class:`~mongomotor.Document`
...
Capped collections
------------------
A :class:`~mongomotor.Document` may use a **Capped Collection** by specifying
...
:attr:`~mongomotor.Document.meta` dictionary, where an index specification may
...
:attr:`auto_create_indexes` (Optional)
    Indicates if the indexes should be create automatically.

.. note:: Unlike MongoEngine, MongoMotor does not create the indexes
    automatically by default. You must to call
    :meth:`~mongomotor.Document.ensure_indexes` to create the indexes.
    It is usually done by calling :meth:`~mongomotor.Document.ensure_indexes`
    at module level after the definition of the document.

.. warning:: :meth:`~mongomotor.Document.ensure_indexes` is synchronous. When
    calling it your program will block.
...
To create a specialised type of a :class:`~mongomotor.Document` you have
defined, you may subclass it and add any extra fields or methods you may need.
As this is new class is not a direct subclass of
:class:`~mongomotor.Document`, it will not be stored in its own collection; it
...
:attr:`abstract` attribute of :attr:`~mongomotor.Document.meta`.
